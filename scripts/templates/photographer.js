...
function photographerTemplate(data) {
  ...
  return { id, name, picture, getUserCardDOM, getPhotographerHeaderDOM };
}
...
function mediaTemplate(mediaData, index) {
  ...
  let liked = false; // Variable pour suivre si l'utilisateur a liké

  heartIcon.addEventListener("click", (event) => {
    event.stopPropagation();
    if (!liked) {
      liked = true;
      mediaData.likes += 1; // Incrémente les likes du média
      likesElement.textContent = mediaData.likes; // Met à jour l'affichage
      updateTotalLikes(); // Met à jour le total des likes
    }
  });

  // Gestion au clavier (Enter ou Space)
